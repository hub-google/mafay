...
local _HM_TargetList = {
  szIniFile = HM.GetAddonInfo().szRootPath .. "HM_TargetList/HM_TargetList.ini",
  szIniFile2 = HM.GetAddonInfo().szRootPath .. "HM_TargetList/HM_TargetList2.ini",
  tFocus = {},
  nFrameFocus = 0,
  nFrameList = 0,
  nFrameAcct = 0,
...
local IsInSameQWG = function(dwPlayer)
...
function _HM_TargetList.GetFocusMenu()
  ...
  return {
    {
      ...
      fnAction = function(d, b)
        HM_SingleFocus.Switch(b)
      end
    },
    {
      szOption = _L["Show buff/cd of single focus"],
      bCheck = true,
      bChecked = HM_SingleFocus.bShowBuffCD,
      fnDisable = function()
        return not HM_TargetMon
      end,
      fnAction = function()
        HM_SingleFocus.bShowBuffCD = not HM_SingleFocus.bShowBuffCD
      end
    },
    {
      szOption = _L["Show move state/buff"],
      bCheck = true,
      bChecked = HM_TargetList.bFocusState,
      fnDisable = function()
        return not HM_TargetList.bShowFocus
      end,
      fnAction = function()
        HM_TargetList.bFocusState = not HM_TargetList.bFocusState
      end
    },
    {
      szOption = _L["Show focused target name"],
      bCheck = true,
      bChecked = HM_TargetList.bFocusTarget2,
      fnDisable = function()
        return not HM_TargetList.bShowFocus
      end,
      fnAction = function()
        HM_TargetList.bFocusTarget2 = not HM_TargetList.bFocusTarget2
      end
    },
    {
      szOption = _L["Auto focus enemy in arena/qiwuge"],
      bCheck = true,
      bChecked = HM_TargetList.bAutoArena,
      fnDisable = function()
        return not HM_TargetList.bShowFocus
      end,
      fnAction = function()
        HM_TargetList.bAutoArena = not HM_TargetList.bAutoArena
      end
    },
    {
      szOption = _L["Auto focus big/camp boss"],
      bCheck = true,
      bChecked = HM_TargetList.bAutoBigBoss,
      fnDisable = function()
        return not HM_TargetList.bShowFocus and HM_Camp
      end,
      fnAction = function()
        HM_TargetList.bAutoBigBoss = not HM_TargetList.bAutoBigBoss
      end
    },
    {
      szOption = _L["Use old focused interface"],
      bCheck = true,
      bChecked = HM_TargetList.bFocusOld3,
      fnDisable = function()
        return not HM_TargetList.bShowFocus
      end,
      fnAction = function()
    ...
    },
    {
      ...
      end,
      {
        ...
        UserData = 1,
        bChecked = n == 1,
        fnAction = _HM_TargetList.AdjustMaxFocus
      },
      {
        szOption = "2",
        bCheck = true,
    ...
    {
      szOption = _L["Auto focus specified NPC"],
      fnAction = function()
        HM.OpenPanel(_L["Focus/TargetList"])
      end
    },
    {
      szOption = _L["<Shift-Click to add focus>"],
      bCheck = true,
      bChecked = HM_TargetList.bAltFocus,
      fnAction = function(d, b)
        HM_TargetList.bAltFocus = b
      end
    },
    {
      szOption = _L["Show skill CD on focus"],
      bCheck = true,
      bChecked = HM_TargetList.bFocusCD,
      fnAction = function(d, b)
        HM_TargetList.bFocusCD = b
      end
    }
  }
end
function _HM_TargetList.AdjustMaxFocus(n)
  if HM_TargetList.nMaxFocus ~= n then
    HM_TargetList.nMaxFocus = n
    while n < #_HM_TargetList.tFocus do
      table.remove(_HM_TargetList.tFocus, 1)
    end
  end
end
function _HM_TargetList.IsFocus(dwID)
...
function _HM_TargetList.AddFocus(dwID, bAuto)
...
function _HM_TargetList.DelFocus(dwID)
  for k, v in ipairs(_HM_TargetList.tFocus) do
    if v == dwID then
...
function _HM_TargetList.SetPersistFocus(dwID)
...
function _HM_TargetList.SwitchFocus(dwID)
  if _HM_TargetList.IsFocus(dwID) then
...
function _HM_TargetList.SetFocus()
...
function _HM_TargetList.SelFocus()
...
function _HM_TargetList.GetFocusItemMenu(dwID)
  if _HM_TargetList.IsFocus(dwID) then
    return {
      szOption = _L["Remove from HM focus"],
      fnAction = function()
        _HM_TargetList.DelFocus(dwID)
      end
    }
...
function _HM_TargetList.GetSimpleNum(n)
  if n < 10000 then
    return tostring(n)
...
function _HM_TargetList.GetForceFontColor(tar, myID, bFocus, bAlone)
  ...
  if not bFocus and HM_TargetList.bListWhite and HM_TargetList.nListMode ~= 1 and HM_TargetList.nListMode ~= 4 then
    return 255, 255, 255
  end
  if IsEnemy(myID, tar.dwID) then
    return 255, 126, 126
...
function _HM_TargetList.GetBackImageFrame(tar, myID)
...
function _HM_TargetList.UpdateFocusGPS(h, tar)
  local me = GetClientPlayer()
...
function _HM_TargetList.GetSkillPrepareState(tar)
  local _, dwSkillID, dwLevel, fP = tar.GetSkillOTActionState()
  if (not dwSkillID or dwSkillID == 0) and IsPlayer(tar.dwID) and tar.GetSkillOTActionState() == 1 then
...
function _HM_TargetList.UpdateFocusMana(h, tar)
  local hImg, hText = h:Lookup("Image_Mana"), h:Lookup("Text_Mana")
  local szSkill, fP = _HM_TargetList.GetSkillPrepareState(tar)
  if not szSkill and IsPlayer(tar.dwID) and tar.GetSkillOTActionState() == 2 and HM_Target then
  ...
  if not h.bOld then
    if nFrame == 42 then
      nFrame = 213
    elseif nFrame == 86 then
      nFrame = 216
    elseif nFrame == 87 then
      nFrame = 214
...
function _HM_TargetList.UpdateFocusItem(h, tar)
  local me = GetClientPlayer()
...
function _HM_TargetList.NewFocusItem(handle, dwID)
  local h
  ...
  function box.OnItemMouseEnter()
    this:SetObjectMouseOver(1)
    if this.dwID then
      OutputBuffTip(this.dwOwner, this.dwID, this.nLevel, 1, false, 0)
    end
  end
  function box.OnItemMouseLeave()
    this:SetObjectMouseOver(0)
    HideTip()
  end
  return h
end
function _HM_TargetList.UpdateFocusItems(handle)
  local tFocus = {}
  for k, v in ipairs(_HM_TargetList.tFocus) do
...
function _HM_TargetList.HookTargetMenu()
  Target_AppendAddonMenu({
    function(dwID)
      return {
        _HM_TargetList.GetFocusItemMenu(dwID),
        {
          szOption = _L["Set as persist focus"],
          fnAction = function()
            _HM_TargetList.SetPersistFocus(dwID)
          end
        },
        {
          szOption = _L["Lock as single focus"],
          fnAction = function()
            HM_SingleFocus.Lock(dwID)
          end
        }
      }
    end
...
function _HM_TargetList.SetFarThreshold()
...
function _HM_TargetList.GetCustomMenu()
  local m1 = {
    szOption = _L["Enable custom option"],
    bCheck = true,
    bChecked = _HM_TargetList.bCustom,
    fnAction = function(d, b)
  ...
  for k, v in pairs(HM_TargetList.tCustomName) do
    table.insert(m2, {
      ...
      bChecked = v,
      fnAction = function(d, b)
        HM_TargetList.tCustomName[k] = b
      end,
      {
        szOption = _L.Remove,
        fnAction = function()
          HM_TargetList.tCustomName[k] = nil
        end
      }
    })
  end
  ...
  for k, v in pairs(HM_TargetList.tCustomTong) do
    table.insert(m2, {
      ...
      bChecked = v,
      fnAction = function(d, b)
        HM_TargetList.tCustomTong[k] = b
      end,
      {
        szOption = _L.Remove,
        fnAction = function()
          HM_TargetList.tCustomTong[k] = nil
        end
      }
    })
  end
  table.insert(m1, m2)
  local m2 = {
    szOption = _L["School force"]
  }
  for k, v in pairs(g_tStrings.tForceTitle) do
    table.insert(m2, {
      szOption = v,
      bCheck = true,
      bChecked = HM_TargetList.tCustomForce[k] == true,
      fnAction = function(d, b)
        HM_TargetList.tCustomForce[k] = b
      end
    })
  end
  table.insert(m1, m2)
  ...
  for k, v in pairs(HM_TargetList.tCustomSave) do
    table.insert(m2, {
      ...
      },
      {
        szOption = _L.Remove,
        fnAction = function()
          HM_TargetList.tCustomSave[k] = nil
        end
      }
    })
  end
...
function _HM_TargetList.GetListMenu()
  local m0 = {}
  local m1 = {
    szOption = _L["View mode of list"]
  }
  for k, v in ipairs(_HM_TargetList.tListMode) do
    table.insert(m1, {
      ...
      bCheck = true,
      bMCheck = true,
      bChecked = k == HM_TargetList.nListMode,
      fnAction = function()
  ...
  table.insert(m0, m1)
  table.insert(m0, {
    szOption = _L["View options of list"],
    {
      szOption = _L["Show image of HP"],
      bCheck = true,
      bChecked = HM_TargetList.bListImage,
      fnAction = function(d, b)
        HM_TargetList.bListImage = b
      end
    },
    {
      szOption = _L["Show level"],
      bCheck = true,
      bChecked = HM_TargetList.tShowMode.bLevel,
      fnAction = function(d, b)
        HM_TargetList.tShowMode.bLevel = b
      end
    },
    {
      szOption = _L["Show distance"],
      bCheck = true,
      bChecked = HM_TargetList.tShowMode.bDistance,
      fnAction = function(d, b)
        HM_TargetList.tShowMode.bDistance = b
      end
    },
    {
      szOption = _L["Show player school"],
      bCheck = true,
      bChecked = HM_TargetList.tShowMode.bForce,
      fnAction = function(d, b)
        HM_TargetList.tShowMode.bForce = b
      end
    },
    {
      szOption = _L["Show only up to 25"],
      bCheck = true,
      bChecked = HM_TargetList.tShowMode.bOnly25,
      fnAction = function(d, b)
        HM_TargetList.tShowMode.bOnly25 = b
      end
    }
  })
  table.insert(m0, {
    szOption = _L["Set list order"],
    {
      szOption = _L["Not sort"],
      bCheck = true,
      bMCheck = true,
      bChecked = HM_TargetList.nSortType2 == 0,
      fnAction = function()
        HM_TargetList.nSortType2 = 0
      end
    },
    {
      szOption = _L["Priority less HP"],
      bCheck = true,
      bMCheck = true,
      bChecked = HM_TargetList.nSortType2 == 1,
      fnAction = function()
        HM_TargetList.nSortType2 = 1
      end,
      {
        szOption = _L["Rear not oriented"],
        bCheck = true,
        bChecked = HM_TargetList.bDownFace2,
        fnAction = function(d, b)
          HM_TargetList.bDownFace2 = b
        end
      },
      {
        szOption = _L.Rear .. HM_TargetList.nFarThreshold .. _L["feet far"],
        bCheck = true,
        bChecked = HM_TargetList.bDownFar,
        fnAction = function(d, b)
          HM_TargetList.bDownFar = b
        end,
        {
          szOption = _L["Edit distance"],
          fnAction = _HM_TargetList.SetFarThreshold
        }
      }
    },
    {
      szOption = _L["Priority closer"],
      bCheck = true,
      bMCheck = true,
      bChecked = HM_TargetList.nSortType2 == 2,
      fnAction = function()
        HM_TargetList.nSortType2 = 2
      end
    },
    {bDevide = true},
    {
      szOption = _L["Sticky treat player"],
      bCheck = true,
      bChecked = HM_TargetList.bUpTreat,
      fnAction = function(d, b)
        HM_TargetList.bUpTreat = b
      end
    },
    {
      szOption = _L["Rear dead player"],
      bCheck = true,
      bChecked = HM_TargetList.bDownDeath,
      fnAction = function(d, b)
        HM_TargetList.bDownDeath = b
      end
    }
  })
  local m1 = {
    szOption = _L["Temporarily filter NPC"],
    {
      szOption = _L["* New *"],
      fnAction = function()
  ...
  for k, v in pairs(_HM_TargetList.tFilterNpc) do
    table.insert(m1, {
      ...
      bChecked = v,
      fnAction = function(d, b)
        _HM_TargetList.tFilterNpc[k] = b
      end,
      {
        szOption = _L.Remove,
        fnAction = function()
          _HM_TargetList.tFilterNpc[k] = nil
        end
      }
    })
  end
  ...
  table.insert(m0, {
    szOption = _L["Keep foucs in list"],
    bCheck = true,
    bChecked = HM_TargetList.bListFocus,
    fnDisable = function()
      return not HM_TargetList.bShowList
    end,
    fnAction = function()
      HM_TargetList.bListFocus = not HM_TargetList.bListFocus
    end
  })
  table.insert(m0, {
    szOption = _L["Use white text in list"],
    bCheck = true,
    bChecked = HM_TargetList.bListWhite,
    fnDisable = function()
      return not HM_TargetList.bShowList
    end,
    fnAction = function()
      HM_TargetList.bListWhite = not HM_TargetList.bListWhite
    end
  })
  return m0
end
function _HM_TargetList.UpdateListTitle()
  if _HM_TargetList.frame then
...
function _HM_TargetList.UpdateList()
...
function _HM_TargetList.UpdateListScroll()
  local win = _HM_TargetList.frame:Lookup("Wnd_List")
  ...
  if nStep < scroll:GetScrollPos() then
    scroll:SetScrollPos(nStep)
  end
end
function _HM_TargetList.GetRelAngle(me, tar)
...
function _HM_TargetList.IsTreatForce(dwForceID)
  return dwForceID == 2 or dwForceID == 5 or dwForceID == 6
end
function _HM_TargetList.ListItemCompare(a, b)
...
function _HM_TargetList.CheckListItem(tar, nMode)
...
function _HM_TargetList.UpdateListItems(handle)
  local aList = {}
  ...
  for _, v in ipairs(aList) do
    if _HM_TargetList.CheckListItem(v, nMode) then
      local item = {
        dwID = v.dwID,
        nMoveState = v.nMoveState,
        szName = HM.GetTargetName(v),
        nLevel = v.nLevel,
  ...
  for k, v in ipairs(aItem) do
    ...
    if HM_TargetList.tShowMode.bTTname then
      local tar = HM.GetTarget(v.dwID)
      if tar then
        local ttar = GetTargetHandle(tar.GetTarget())
        if ttar then
          if ttar.dwID == me.dwID then
            szText = "->[ÎÒ]" .. szText
          else
            szText = "->[" .. HM.GetTargetName(ttar) .. "]" .. szText
          end
        end
      end
    end
    if HM_TargetList.tShowMode.bDistance then
      szText = szText .. "<" .. string.format("%.f", v.nDis) .. ">"
    end
...
function _HM_TargetList.Switch(bShow)
  if bShow == nil then
  ...
  if HM_TargetList.bShow2 then
    frame = frame or Wnd.OpenWindow(_HM_TargetList.szIniFile, "HM_TargetList")
  elseif frame then
    Wnd.CloseWindow(frame)
    _HM_TargetList.frame = nil
  end
end
function _HM_TargetList.MonitorArena(szMsg)
...
function _HM_TargetList.GetAcctInfo()
...
function _HM_TargetList.ShowAcctInfo()
...
function _HM_TargetList.UpdateAcctInfo()
...
function _HM_TargetList.UpdateAnchor()
...
function _HM_TargetList.UpdateSize(bFocusOnly)
  if not HM_TargetList.bShow2 then
    return
  end
...
function HM_TargetList.OnFrameCreate()
...
function HM_TargetList.OnFrameDragEnd()
  this:CorrectPos()
  HM_TargetList.tAnchor = GetFrameAnchor(this)
end
function HM_TargetList.OnFrameBreathe()
  if not GetClientPlayer() then
    return
  end
  local nFrame = GetLogicFrameCount()
  if not _HM_TargetList.bCollapse and HM_TargetList.bShowFocus and not HM_TargetList.bSplitFocus and nFrame - _HM_TargetList.nFrameFocus > 1 then
  ...
  if not _HM_TargetList.bCollapse and HM_TargetList.bShowList and nFrame - _HM_TargetList.nFrameList > 5 then
  ...
  if not _HM_TargetList.bCollapse and HM_TargetList.bShowAcct and nFrame - _HM_TargetList.nFrameAcct >= GLOBAL.GAME_FPS then
...
function HM_TargetList.OnEvent(event)
      ...
      do
        ...
        if frm then
          ...
          if not hnd.OnItemLButtonDown then
            function hnd.OnItemLButtonDown()
            ...
            if v == "TargetTarget" then
              function hnd.OnItemRButtonDown()
                local menu = {}
                table.insert(menu, _HM_TargetList.GetFocusItemMenu(frm.dwID))
                table.insert(menu, {
                  szOption = _L["Set as persist focus"],
                  fnAction = function()
                    _HM_TargetList.SetPersistFocus(frm.dwID)
                  end
                })
                table.insert(menu, {
                  szOption = _L["Lock as single focus"],
                  fnAction = function()
                    HM_SingleFocus.Lock(frm.dwID)
                  end
                })
                PopupMenu(menu)
                return 2
  ...
  elseif event == "PLAYER_ENTER_SCENE" or event == "PLAYER_LEAVE_SCENE" or event == "NPC_ENTER_SCENE" or event == "NPC_LEAVE_SCENE" then
    if HM_TargetList.bAutoArena and event == "PLAYER_ENTER_SCENE" and not _HM_TargetList.IsFocus(arg0) and IsEnemy(GetClientPlayer().dwID, arg0) and (_HM_TargetList.bInArena or IsInSameQWG(arg0)) then
    ...
    if HM_TargetList.bAutoBigBoss and not _HM_TargetList.bInCopy and HM_Camp and event == "NPC_ENTER_SCENE" and not _HM_TargetList.IsFocus(arg0) and HM_Camp.IsCareNpc(GetNpc(arg0)) then
      _HM_TargetList.AddFocus(arg0, true)
    end
...
function HM_TargetList.OnLButtonClick()
  ...
  elseif szName == "Btn_Setting" then
    local m0 = {}
    table.insert(m0, {
      szOption = _L["Show focus target"],
      bCheck = true,
      bChecked = HM_TargetList.bShowFocus,
      fnAction = function(d, b)
        if _HM_TargetList.ui then
    ...
    table.insert(m0, {
      szOption = _L["Show target list"],
      bCheck = true,
      bChecked = HM_TargetList.bShowList,
      fnAction = function(d, b)
        if _HM_TargetList.ui then
    ...
    table.insert(m0, {
      szOption = _L["Show player statistics"],
      bCheck = true,
      bChecked = HM_TargetList.bShowAcct,
      fnAction = function(d, b)
    ...
    table.insert(m0, {
      szOption = _L["Split focus panel"],
      bCheck = true,
      bChecked = HM_TargetList.bSplitFocus,
      fnAction = function(d, b)
    ...
    table.insert(m0, {bDevide = true})
    table.insert(m0, {
      szOption = _L["Enable double click to focus fire"],
      bCheck = true,
      bChecked = HM_TargetList.bJihuo,
      fnDisable = function()
        return HM_Marker == nil
      end,
      fnAction = function()
        HM_TargetList.bJihuo = not HM_TargetList.bJihuo
      end
    })
    table.insert(m0, {
      szOption = _L["Publish nearby stats"],
      fnDisable = function()
        return HM_RedName == nil
      end,
      {
        szOption = _L["Statistical by camp"],
        fnAction = function()
          HM_RedName.ShowAroundInfo(0)
        end
      },
      {
        szOption = _L["Statistical by school"],
        fnAction = function()
          HM_RedName.ShowAroundInfo(1)
        end
      },
      {
        szOption = _L["Statistical by guild"],
        fnAction = function()
          HM_RedName.ShowAroundInfo(2)
        end
      }
    })
    table.insert(m0, {
      szOption = _L["Background image opacity("] .. HM_TargetList.nAlphaBg .. ")",
      fnAction = function()
        ...
        end, nil, HM_TargetList.nAlphaBg / 100, _L["Adjust opacity of background"], {
          fX,
          fY,
          fX + 1,
          fY + 1
...
function HM_TargetList.OnCheckBoxCheck()
  local szName = this:GetName()
...
function HM_TargetList.OnCheckBoxUncheck()
  local szName = this:GetName()
  if szName == "Check_Minimize" then
...
function HM_TargetList.OnMouseLeave()
  if this:GetName() == "Scroll_List" then
...
function HM_TargetList.OnItemMouseEnter()
  local szName = this:GetName()
...
function HM_TargetList.OnItemMouseLeave()
  local szName = this:GetName()
  if szName == "Image_PlayerBg" or szName == "Text_Mana" then
    local hMana = this:GetParent():Lookup("Text_Mana")
    if hMana then
...
function HM_TargetList.OnItemLButtonDown()
  if this:GetName() == "Handle_ATotal" then
    PopupMenu({
      {
        szOption = _L["Publish nearby stats"],
        fnAction = _HM_TargetList.ShowAcctInfo
      }
    })
...
function HM_TargetList.OnItemRButtonDown()
  if this:GetName() == "Handle_ATotal" then
    PopupMenu({
      {
        szOption = _L["Publish nearby stats"],
    ...
    do
      ...
      if IsPlayer(this.dwID) then
        do
          ...
          if me.IsInParty() and InsertMarkMenu and me.dwID == GetClientTeam().GetAuthorityInfo(TEAM_AUTHORITY_TYPE.MARK) then
      ...
      elseif this.bList then
        do
          local szName = this.szName
          table.insert(m0, {
            szOption = _L["Filter named this NPC"],
            fnAction = function()
              _HM_TargetList.tFilterNpc[szName] = true
            end
          })
        end
      end
...
function HM_TargetList.OnItemLButtonDBClick()
...
function HM_TargetList.OnItemMouseWheel()
...
function HM_SplitFocus.UpdateAnchor(frame)
...
function HM_SplitFocus.OnFrameCreate()
...
function HM_SplitFocus.OnFrameBreathe()
  if not GetClientPlayer() then
    return
  end
  local nFrame = GetLogicFrameCount()
...
function HM_SplitFocus.OnFrameDragEnd()
  this:CorrectPos()
  HM_SplitFocus.tAnchor = GetFrameAnchor(this)
end
function HM_SplitFocus.OnEvent(event)
...
function HM_SplitFocus.Switch(bEnable)
  if bEnable ~= nil then
  ...
  elseif not frame then
    Wnd.OpenWindow(HM.GetAddonInfo().szRootPath .. "HM_TargetList/HM_SplitFocus.ini", "HM_SplitFocus")
  end
end
HM_SingleFocus = {
  bEnable2 = true,
...
function HM_SingleFocus.UpdateAnchor(frame)
...
function HM_SingleFocus.GetListBox(hList, nS)
  local nCount = hList:GetItemCount()
  if nCount > hList.nIndex then
    nCount = hList.nIndex
  else
...
function HM_SingleFocus.OnFrameCreate()
...
function HM_SingleFocus.OnFrameBreathe()
...
function HM_SingleFocus.OnFrameDragEnd()
  this:CorrectPos()
  HM_SingleFocus.tAnchor = GetFrameAnchor(this)
end
function HM_SingleFocus.OnEvent(event)
  if event == "ON_ENTER_CUSTOM_UI_MODE" or event == "ON_LEAVE_CUSTOM_UI_MODE" then
    UpdateCustomModeWindow(this)
  elseif event == "UI_SCALED" then
    HM_SingleFocus.UpdateAnchor(this)
...
function HM_SingleFocus.Switch(bEnable)
  if bEnable ~= nil then
  ...
  elseif not frame then
    Wnd.OpenWindow(HM.GetAddonInfo().szRootPath .. "HM_TargetList/HM_SingleFocus.ini", "HM_SingleFocus")
  end
end
function HM_SingleFocus.Lock(dwID)
...
_HM_TargetList.PS = {}
function _HM_TargetList.PS.OnPanelDeactive(frame)
  _HM_TargetList.ui = nil
end
function _HM_TargetList.GetPersistNpcText()
  local tName = {}
...
function _HM_TargetList.PS.OnPanelActive(frame)
  local ui, nX = HM.UI(frame), 0
  ui:Append("Text", {
    x = 0,
    y = 0,
    txt = _L["Feature setting"],
    font = 27
  })
  nX = ui:Append("WndCheckBox", "Check_Show", {
    x = 10,
    y = 28,
    checked = HM_TargetList.bShow2
  ...
  ui:Append("Text", {
    txt = HM.GetHotKey("ShowTL", false) .. _L[") "],
    x = nX,
    y = 27
  })
  ui:Append("WndCheckBox", "Check_Focus", {
    x = 10,
    y = 56,
    checked = HM_TargetList.bShowFocus
  ...
  ui:Append("WndCheckBox", "Check_List", {
    x = 10,
    y = 84,
    checked = HM_TargetList.bShowList
  ...
  nX = ui:Append("WndComboBox", {
    x = 10,
    y = 114,
    txt = _L["Foucs setting"]
  }):Menu(_HM_TargetList.GetFocusMenu):Pos_()
  ui:Append("WndComboBox", {
    x = nX + 20,
    y = 114,
    txt = _L["List setting"]
  }):Menu(_HM_TargetList.GetListMenu)
  _HM_TargetList.ui = ui
  ui:Append("Text", {
    x = 0,
    y = 150,
    txt = _L["Auto focus specified NPC"] .. _L["("] .. _L["One per line"] .. _L[") "],
    font = 27
  })
  nX = ui:Append("WndEdit", "Edit_Npc", {
    x = 15,
    y = 178,
    limit = 4096,
    h = 60,
    w = 380,
    multi = true
  }):Text(_HM_TargetList.GetPersistNpcText()):Enable(HM_TargetList.bShowFocus):Change(function(szText)
    local t = {}
  ...
  nX = ui:Append("Text", {
    x = 10,
    y = 278,
    txt = _L["1. Support to set/select focus by hotkey, "]
  }):Pos_()
  ui:Append("WndButton", {
    txt = _L["Enter setting"],
    x = nX + 5,
    y = 280
  }):AutoSize(8):Click(HM.SetHotKey)
  ui:Append("Text", {
    x = 10,
    y = 306,
    txt = _L["2. Hotkey supported when mouse move over the npc/player of scene, "]
  })
  ui:Append("Text", {
    x = 10,
    y = 334,
    txt = _L["3. Press SHIFT and click target/targettarget/target list can add/remove focus"]
  })
  ui:Append("Text", {
    x = 10,
    y = 362,
    txt = _L["4. Send 33 to around player can add as focus"]
  })
end
function _HM_TargetList.PS.OnPlayerMenu()
  return {
...
HM.AppendTraceMenu(function()
  return {
    szOption = _L["HM, focus targetlist"],
    bCheck = true,
    bChecked = HM_TargetList.bShow2,
    fnAction = _HM_TargetList.Switch
  }
